	...
	public static Inventory GUI(Player p) {
		...
		if (PerkManager.checkPerkUnlocked(p, "jellylegs")) {
			if (PerkManager.checkPerkActive(p, "jellylegs")) {
				Utils.createItem(inv, "GREEN_STAINED_GLASS_PANE", 1, 1, "&d&lJellyLegs &a&lActive",
						"&7Disable all fall damage with this perk!", "&e one of the best perks on the server",
						"&efor PvP!", "", "&aCurrently Activated &8(&7Click to &cDisable&8)");
			} else {
				Utils.createItem(inv, "YELLOW_STAINED_GLASS_PANE", 1, 1, "&d&lJellyLegs &e&lDisabled",
						"&7Disable all fall damage with this perk!", "&e one of the best perks on the server",
			...
			Utils.createItem(inv, "BLACK_STAINED_GLASS_PANE", 1, 1, "&d&lJellyLegs &8(&7LOCKED&8)",
					"&7Disable all fall damage with this perk!", "&e one of the best perks on the server",
					"&efor PvP!", "", "&9Requires &fChicken &7rank to unlock!");
		}
		...
		if (ChatColor.stripColor(clicked.getItemMeta().getDisplayName()).equalsIgnoreCase("JellyLegs (LOCKED)")) {
			if (RankManager.getRank(p) >= 1) {
				if (Main.econ.getBalance(p) >= 5000) {
					...
					p.sendMessage(Utils.chat("&9&lPerks &8> &7You do not have sufficent &9Perk Credits &7to unlock this &eperk&7!"));
				}
			} else {
				p.sendMessage(Utils.chat("&9&lPerks &8> &7You must have &f&lChicken &7unlocked to purchase this perk!"));
			}
		} 
	}
}